      ...
      {lightboxIndex !== null && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center" onClick={() => setLightboxIndex(null)}>
          ...
          <button
            ...
            onClick={e => { e.stopPropagation(); setLightboxIndex(i => (i! > 0 ? i! - 1 : imageUrls.length - 1)) }}
          >
            <ChevronRight className="w-8 h-8" />
          </button>
          <button
            className="absolute left-4 top-1/2 -translate-y-1/2 text-white/80 hover:text-white p-2"
            onClick={e => { e.stopPropagation(); setLightboxIndex(i => (i! < imageUrls.length - 1 ? i! + 1 : 0)) }}
          >
          ...
          />
          <div className="absolute bottom-4 text-white/60 text-sm">{lightboxIndex + 1} / {imageUrls.length}</div>
        </div>
      )}
    </>
  )
