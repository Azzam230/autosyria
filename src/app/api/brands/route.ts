...
export async function POST(request: Request) {
  ...
  const { data, error } = await getSupabase().from("brands").insert([{ name, logo_url: logo_url || null }]).select().single()
  if (error) {
...
export async function PUT(request: Request) {
  ...
  const { data, error } = await getSupabase().from("brands").update({ name, logo_url: logo_url || null }).eq("id", id).select().single()
  if (error) {
    if (error.code === "23505") return NextResponse.json({ error: "الماركة موجودة مسبقاً" }, { status: 409 })
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
